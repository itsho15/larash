...
class LumenHelper {
	...
	public static function getNamespace() {
		try {
			return explode('\\', with(new \ReflectionClass(get_called_class()))->getNamespaceName())[0];
		} catch (\Exception $e) {
			wp_die($e);
		}
	}
	 ...
	 * @return mixed
	 */
	public static function plugin($namespace = null) {
		$namespace = !is_null($namespace) ? $namespace : self::getNamespace();
		if (isset(self::$app_instances[$namespace])) {
			try {
	...
	public function asset($url) {
		return plugin_dir_url($this->base_path($this->config('app.plugin_file'))) . $url;
	}
	...
	/**
	 * Return the current request from the application. @return \Illuminate\Http\Request
	 */
	function request() {
	...
	/**
	 * Get an Instance of A Recursive Collection
	 * @param array $array
	 * @return mixed
	 */
